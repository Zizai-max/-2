    ...
    <main className="case-page" style={projectStyle}>
      ...
      <section className="case-hero">
        <div className="case-hero-bg">
          {project.slug !== "ai-application" ? <img src={withBasePath(project.coverImage)} alt="" aria-hidden="true" decoding="async" /> : null}
        </div>
            ...
            <div className={`case-gallery shell gallery-${Math.min(section.images.length, 3)}${section.galleryStyle || project.galleryStyle ? ` gallery-${section.galleryStyle ?? project.galleryStyle}` : ""}`}>
              {section.images.map((image, imageIndex) => (
                <figure
                  ...
                  key={image}
                >
                  <img loading="lazy" decoding="async" src={withBasePath(image)} alt={`${project.title} - ${section.title}设计展示 ${imageIndex + 1}`} />
